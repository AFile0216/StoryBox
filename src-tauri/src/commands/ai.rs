...
pub struct GenerateRuntimeConfigDto {
    ...
    pub width_node_id: Option<String>,
    pub height_node_id: Option<String>,
    pub seed_node_id: Option<String>,
    pub steps_node_id: Option<String>,
    pub cfg_node_id: Option<String>,
    pub denoise_node_id: Option<String>,
}

#[derive(Debug, Serialize, Deserialize)]
pub struct ConfigureProviderRuntimeDto {
    pub provider: String,
    pub api_key: String,
...
pub async fn set_api_key(provider: String, api_key: String) -> Result<(), String> {
    configure_provider_runtime(ConfigureProviderRuntimeDto {
        provider,
        api_key,
        base_url: None,
        upload_base_url: None,
...
#[tauri::command]
pub async fn configure_provider_runtime(config: ConfigureProviderRuntimeDto) -> Result<(), String> {
    info!(
        "Configuring provider runtime: {}, base_url={:?}, upload_base_url={:?}",
        config.provider, config.base_url, config.upload_base_url
    );

    let registry = get_registry();
    let resolved_provider = registry
        .get_provider(config.provider.as_str())
...
pub async fn submit_generate_image_job(
    ...
    let req = GenerateRequest {
        ...
        runtime_config: request.runtime_config.map(|config| GenerateRuntimeConfig {
            ...
            width_node_id: config.width_node_id.unwrap_or_default(),
            height_node_id: config.height_node_id.unwrap_or_default(),
            seed_node_id: config.seed_node_id.unwrap_or_default(),
            steps_node_id: config.steps_node_id.unwrap_or_default(),
            cfg_node_id: config.cfg_node_id.unwrap_or_default(),
            denoise_node_id: config.denoise_node_id.unwrap_or_default(),
        }),
    };
