...
const DEFAULT_FAL_QUEUE_BASE_URL: &str = "https://queue.fal.run";
...
pub struct FalProvider {
    ...
    queue_base_url: Arc<RwLock<String>>,
}
...
impl FalProvider {
    pub fn new() -> Self {
        Self {
            ...
            queue_base_url: Arc::new(RwLock::new(DEFAULT_FAL_QUEUE_BASE_URL.to_string())),
        }
    }
    ...
    fn resolve_poll_endpoints(
        ...
        let status_endpoint = format!(
            "{}/{}/requests/{}/status",
            queue_base_url, model_path, request_id
        );
        let result_endpoint = format!(
            "{}/{}/requests/{}",
            queue_base_url, model_path, request_id
        );
        let fallback_status_endpoint = format!("{}/requests/{}/status", queue_base_url, request_id);
        let fallback_result_endpoint = format!("{}/requests/{}", queue_base_url, request_id);
...
impl AIProvider for FalProvider {
    ...
    async fn configure_runtime(&self, config: ProviderRuntimeConfig) -> Result<(), AIError> {
        ...
        let normalized_base_url = config
            .base_url
            .unwrap_or_else(|| DEFAULT_FAL_QUEUE_BASE_URL.to_string())
            .trim()
            .trim_end_matches('/')
            .to_string();
        let next_base_url = if normalized_base_url.is_empty() {
            DEFAULT_FAL_QUEUE_BASE_URL.to_string()
        } else {
            normalized_base_url
        };
        let mut queue_base_url = self.queue_base_url.write().await;
        *queue_base_url = next_base_url;
        Ok(())
    }
    ...
    async fn submit_task(&self, request: GenerateRequest) -> Result<ProviderTaskSubmission, AIError> {
        ...
        let queue_base_url = self.queue_base_url.read().await.clone();
        let submit_endpoint = format!("{}/{}", queue_base_url, model_path);
        let mut input = json!({
    ...
    async fn poll_task(&self, handle: ProviderTaskHandle) -> Result<ProviderTaskPollResult, AIError> {
        ...
        let (status_endpoints, result_endpoints) =
            Self::resolve_poll_endpoints(
                queue_base_url.as_str(),
                handle.task_id.as_str(),
                handle.metadata.as_ref(),
            );
