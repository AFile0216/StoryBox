...
const DEFAULT_TASK_BASE_URL: &str = "https://api.kie.ai";
const DEFAULT_FILE_BASE_URL: &str = "https://kieai.redpandaai.co";
...
pub struct KieProvider {
    ...
    task_base_url: Arc<RwLock<String>>,
    file_base_url: Arc<RwLock<String>>,
}
...
impl KieProvider {
    pub fn new() -> Self {
        Self {
            ...
            task_base_url: Arc::new(RwLock::new(DEFAULT_TASK_BASE_URL.to_string())),
            file_base_url: Arc::new(RwLock::new(DEFAULT_FILE_BASE_URL.to_string())),
        }
    }
    ...
    async fn upload_reference_image(
        ...
        let file_base_url = self.file_base_url.read().await.clone();
    ...
    async fn create_task(
        ...
        let task_base_url = self.task_base_url.read().await.clone();
...
impl AIProvider for KieProvider {
    ...
    async fn configure_runtime(&self, config: ProviderRuntimeConfig) -> Result<(), AIError> {
        ...
        let task_base_url = config
            .base_url
            .unwrap_or_else(|| DEFAULT_TASK_BASE_URL.to_string())
            .trim()
            .trim_end_matches('/')
            .to_string();
        let file_base_url = config
            .upload_base_url
        ...
        let mut task_base_url_guard = self.task_base_url.write().await;
        *task_base_url_guard = if task_base_url.is_empty() {
            DEFAULT_TASK_BASE_URL.to_string()
        } else {
            task_base_url
        };
        let mut file_base_url_guard = self.file_base_url.write().await;
        *file_base_url_guard = if file_base_url.is_empty() {
