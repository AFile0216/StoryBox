    ...
    async fn generate_via_images_api(
        ...
        if let Some(reference_images) = request.reference_images.as_ref().filter(|items| !items.is_empty()) {
            ...
            if normalized_images.len() == 1 {
                // Single image: send as scalar string (broadest API compatibility)
                body.insert("image".to_string(), json!(normalized_images[0]));
            } else {
                // Multiple images:
                // 1. Send as JSON array under "image" — standard format for most multi-image APIs
                body.insert("image".to_string(), json!(normalized_images));
                // 2. Also send individual image2/image3/... fields — for APIs using numbered params
                for (i, img) in normalized_images.iter().enumerate().skip(1) {
                    body.insert(format!("image{}", i + 1), json!(img));
                }
            }
        }
        ...
        // Standard { data: [{ url, b64_json }] } format
        if let Some(items) = result.get("data").and_then(Value::as_array) {
            if let Some(first) = items.first() {
                if let Some(url) = first.get("url").and_then(Value::as_str).filter(|v| !v.trim().is_empty()) {
                    return Ok(url.to_string());
                }
                if let Some(b64) = first.get("b64_json").and_then(Value::as_str).filter(|v| !v.trim().is_empty()) {
                    return Ok(format!("data:image/png;base64,{}", b64));
                }
            }
        }

        // Fallback: try chat-completions response shape
        if let Some(image_source) = Self::extract_image_from_chat_response(&result) {
            return Ok(image_source);
        }

        Err(AIError::Provider(format!(
            "OpenAI-compatible response has no recognizable image output: {}",
            result
        )))
    }
