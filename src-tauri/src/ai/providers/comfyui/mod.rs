...
impl AIProvider for ComfyUiProvider {
    ...
    async fn generate(&self, request: GenerateRequest) -> Result<String, AIError> {
        ...
        if !runtime.workflow_id.trim().is_empty() {
            Self::set_number_input(&mut workflow, &runtime.workflow_id, &["width"], json!(width));
            Self::set_number_input(&mut workflow, &runtime.workflow_id, &["height"], json!(height));
        }
