...
function inferEdgeRelation(sourceNode: CanvasNode, targetNode: CanvasNode): CanvasEdgeRelation {
  if (
    sourceNode.type === CANVAS_NODE_TYPES.videoStoryboard &&
    targetNode.type === CANVAS_NODE_TYPES.storyboardGen
  ) {
...
function normalizeEdgeData(
  ...
  return {
    relation: data?.relation ?? inferEdgeRelation(sourceNode, targetNode),
    autoGenerated: data?.autoGenerated ?? false,
    color: data?.color ?? null,
    lineStyle: data?.lineStyle ?? null,
    routingStyle: data?.routingStyle ?? 'inherit',
...
function normalizeEdgesWithNodes(rawEdges: CanvasEdge[], nodes: CanvasNode[]): CanvasEdge[] {
  ...
  return rawEdges
    ...
    .map((edge) => ({
      ...
      data: normalizeEdgeData(edge.data, nodeMap.get(edge.source)!, nodeMap.get(edge.target)!),
    }));
}
...
export const useCanvasStore = create<CanvasState>((set, get) => ({
    ...
    set((state) => {
      ...
      const edgeData =
        sourceNode && targetNode
          ? normalizeEdgeData(undefined, sourceNode, targetNode)
          : undefined;
  ...
  toggleGroupCollapsed: (groupNodeId) => {
    const state = get();
    const groupNode = state.nodes.find(
      (node) => node.id === groupNodeId && node.type === CANVAS_NODE_TYPES.group
    );
    ...
    const groupData = groupNode.data as typeof groupNode.data & {
      collapsed?: boolean;
      expandedWidth?: number | null;
      expandedHeight?: number | null;
    };
    const nextCollapsed = !groupData.collapsed;
    const directChildren = state.nodes.filter((node) => node.parentId === groupNodeId);
    const descendantIds = collectNodeIdsWithDescendants(
      state.nodes,
      directChildren.map((node) => node.id)
    );
    const currentSize = getNodeSize(groupNode);
    const restoredWidth = Math.max(220, Math.round(groupData.expandedWidth ?? currentSize.width));
    const restoredHeight = Math.max(140, Math.round(groupData.expandedHeight ?? currentSize.height));
    ...
    const nextNodes = state.nodes.map((node) => {
      if (node.id === groupNodeId) {
        return {
          ...
          style: {
            ...(node.style ?? {}),
            width: nextCollapsed ? restoredWidth : restoredWidth,
            height: nextCollapsed ? 76 : restoredHeight,
          },
          data: {
            ...node.data,
            collapsed: nextCollapsed,
            expandedWidth: restoredWidth,
            expandedHeight: nextCollapsed ? restoredHeight : groupData.expandedHeight ?? restoredHeight,
          },
        };
      }
    ...
    set({
      nodes: nextNodes,
      selectedNodeId: state.selectedNodeId && descendantIds.has(state.selectedNodeId)
        ? groupNodeId
  ...
  updateEdgeData: (edgeId, data) => {
    set((state) => {
      ...
      const nextEdges = state.edges.map((edge) => {
        ...
        const hasDataChange = Object.entries(data).some(([key, value]) =>
          !Object.is((edge.data as Record<string, unknown> | undefined)?.[key], value)
        );
        if (!hasDataChange) {
          return edge;
        }
