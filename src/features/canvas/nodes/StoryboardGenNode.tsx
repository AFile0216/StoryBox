  ...
  const handleGenerate = useCallback(async (previewGridOnly = false) => {
    ...
    try {
      ...
      const allFrameDescriptions = nodeData.frames
        ...
        .map((frame) => frameDescriptionDraftsRef.current[frame.id] ?? frame.description)
        .join('\n');
      const filteredIncomingImages = filterReferencedImages(incomingImages, allFrameDescriptions);

      // 将过滤后的图片和网格图片组合
      const allReferenceImages = [...filteredIncomingImages, gridImageDataUrl];
